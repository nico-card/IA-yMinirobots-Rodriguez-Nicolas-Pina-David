...
=======================================================================================
Ahora con OBSTÁCULOS que el robot no puede atravesar.
"""
...
# ---------------------------------------------------------------------------
# 0. Definición de obstáculos (función solicitada)
# ---------------------------------------------------------------------------
def obstaculos():
    """
    Devuelve un conjunto de coordenadas (x, y) donde el robot no puede pisar.
    Puedes modificar esta función para crear diferentes mapas.
    """
    # Algunas rocas / pilares estratégicos
    obs = set()
    # Pared exterior (opcional, descomentar para encerrar el robot)
    # for i in range(GRID_SIZE):
    #     obs.add((i, 0))
    #     obs.add((i, GRID_SIZE-1))
    #     obs.add((0, i))
    #     obs.add((GRID_SIZE-1, i))

    # Obstáculos internos (un pequeño laberinto)
    for i in range(5, 10):
        obs.add((i, 8))   # fila horizontal de obstáculos
        obs.add((i, 12))
    for j in range(6, 14):
        obs.add((14, j))
        obs.add((6, j))
...
C_OBSTACLE = (80, 40, 40)          # color ladrillo
C_OBSTACLE_LINE = (140, 70, 70)
...
# ---------------------------------------------------------------------------
# 3. Clase Robot (Lógica del entorno virtual) con OBSTÁCULOS
# ---------------------------------------------------------------------------
class Robot:

    def __init__(self):
        # Inicializar obstáculos desde la función externa
        self.obstacles = obstaculos()   # conjunto de (x,y) bloqueados

        # Buscar posición inicial libre (evitar empezar dentro de un obstáculo)
        start_x, start_y = GRID_SIZE // 2, GRID_SIZE // 2
        if (start_x, start_y) in self.obstacles:
            # mover a la primera celda libre que encontremos
            for i in range(GRID_SIZE):
    ...
    def _step(self, forward: bool) -> bool:
        """Desplaza físicamente al robot en la matriz cuidando los límites y obstáculos."""
        dx, dy = DIR_DELTA[self.direction]
        if not forward:
            dx, dy = -dx, -dy  # Invierte las coordenadas para ir en reversa

        nx, ny = self.x + dx, self.y + dy
        # Validación de fronteras y obstáculos
...
def execute_ia_intent(intent: str, robot: Robot) -> str:
    ...
    if intent == "mueve_izquierda":
        ...
        return (
            "↖ Cort: Girar + Avanzar Izquierda" + ("" if ok else " (Bloqueado por obstáculo)")
        )

    elif intent == "mueve_derecha":
        ok = robot.turn_and_move_right()
        return "↗ Cort: Girar + Avanzar Derecha" + ("" if ok else " (Bloqueado por obstáculo)")

    elif intent == "adelante":
        ok = robot.move_forward()
        return "⬆ Cort: Adelante" + ("" if ok else " (Bloqueado por obstáculo)")

    elif intent == "atras":
        ok = robot.move_backward()
        return "⬇ Cort: Atrás" + ("" if ok else " (Bloqueado por obstáculo)")
...
def draw_robot(surf: pygame.Surface, robot: Robot):
    ...
    for i, (tx, ty) in enumerate(robot.trail):
        ...
        factor = (i + 1) / len(robot.trail) if robot.trail else 1
...
def draw_obstacles(surf: pygame.Surface, robot: Robot):
    """Dibuja los obstáculos como bloques con textura de rejilla."""
    for (ox, oy) in robot.obstacles:
        rect = pygame.Rect(ox * CELL_SIZE, oy * CELL_SIZE, CELL_SIZE, CELL_SIZE)
        pygame.draw.rect(surf, C_OBSTACLE, rect)
        # Patrón de rejilla oscura
        for i in range(3):
            pygame.draw.line(surf, C_OBSTACLE_LINE,
                            (rect.x + i*8, rect.y),
                            (rect.x + i*8, rect.y+CELL_SIZE), 1)
            pygame.draw.line(surf, C_OBSTACLE_LINE,
                            (rect.x, rect.y + i*8),
                            (rect.x+CELL_SIZE, rect.y + i*8), 1)
        # Borde exterior
        pygame.draw.rect(surf, (50, 25, 25), rect, 2)
...
def main():
    ...
    pygame.display.set_caption("🤖 Robot Inteligente NLP con Red Neuronal + Obstáculos")
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    ok = robot.move_forward()
                    last_action = "Teclado manual: Adelante" + ("" if ok else " (Obstáculo bloquea)")
                elif event.key == pygame.K_DOWN:
                    ok = robot.move_backward()
                    last_action = "Teclado manual: Atrás" + ("" if ok else " (Obstáculo bloquea)")
    ...
    # --- Parche para comandos cortos: "izquierda" o "derecha" deben ser solo giro ---
            clean_text = raw_text.strip().lower()
            if clean_text in ("izquierda", "izq", "derecha", "der"):
                intent = clean_text  # "izquierda" o "derecha"
                confidence = 1.0      # ficticio, solo para mostrar en la interfaz
                ia_info = f"Cortana (regla directa): '{intent}' | Certeza: 100%"
            else:
        ...
        draw_obstacles(screen, robot)   # <-- dibujar los obstáculos
